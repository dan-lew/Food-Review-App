...
router.get("/getRestaurant", async (req, res) => {
  try {
    let restaurantNames = await Restaurant.find({}).select({
      restaurantName: 1,
    });
...
router.post("/category", auth, async (req, res) => {
  ...
  try {
    let category = req.body.category;
    const restaurants = await Restaurant.find({ category: category }).sort({
      date: -1,
    });
    console.log(restaurants);
    res.json(restaurants);
...
router.post(
  ...
  [
    ...
    check("city").trim().not().isEmpty().withMessage("City is empty"),
    check("country").trim().not().isEmpty().withMessage("Country is empty"),
    check("category")
      .trim()
      .not()
      .isEmpty()
...
router.post("/getrestaurantfood", auth, async (req, res, next) => {
  ...
  let restaurants = await Reviews.find({
    $and: [{ nameOfDish: nameOfDish }, { city: city }],
  });
  ...
  let restaurantInfo = await Restaurant.find({
    restaurantName: restaurantsList,
  });
